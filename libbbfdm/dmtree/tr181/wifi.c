	...
	switch (del_action) {
		case DEL_INST:
			if (((struct wifi_ssid_args *)data)->config_s) {
				...
				dmmap_s = get_dup_section_in_dmmap("dmmap_wireless", "wifi-iface", section_name(((struct wifi_ssid_args *)data)->config_s));
...
{
	return get_net_device_status(((struct wifi_ssid_args *)data)->ifname, value);
}
...
static int set_access_point_security_modes(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	...
	switch (action) {
		case VALUECHECK:
			...
			return 0;
		case VALUESET:
			set_security_mode((((struct wifi_acp_args *)data)->sections)->config_section, map_s, (((struct wifi_acp_args *)data)->sections)->dmmap_section, value);
			return 0;
	}
	return 0;
}
...
static int set_access_point_security_shared_key(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	...
	switch (action) {
		...
		case VALUESET:
			...
			dmuci_set_value_by_section((((struct wifi_acp_args *)data)->sections)->dmmap_section, "key", value);
...
static int set_WiFiAccessPointSecurity_SAEPassphrase(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	...
	switch (action)	{
		...
		case VALUESET:
			// Set the key in dmmap as well
			dmuci_set_value_by_section((((struct wifi_acp_args *)data)->sections)->dmmap_section, "SAEPassphrase", value);
...
static int set_WiFiEndPointProfileSecurity_ModeEnabled(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	...
	switch (action) {
		case VALUECHECK:
			...
			return 0;
		case VALUESET:
			set_security_mode((struct uci_section *)data, map_s, NULL, value);
			return 0;
	}
	return 0;
}
