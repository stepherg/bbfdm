...
static int addObjDHCPv6Client(char *refparam, struct dmctx *ctx, void *data, char **instance)
{
	...
	char dhcpv6_client_key[32] = {0};

	snprintf(dhcpv6_client_key, sizeof(dhcpv6_client_key), "dhcpv6_client_%s", *instance);

	dmuci_add_section("network", "interface", &s);
	dmuci_rename_section_by_section(s, dhcpv6_client_key);
	...
	dmuci_set_value_by_section(dmmap_sect, "section_name", dhcpv6_client_key);
	...
	switch (del_action) {
		...
		case DEL_ALL:
			uci_foreach_option_eq_safe("network", "interface", "proto", "dhcp", stmp, s) {
				...
				get_dmmap_section_of_config_section("dmmap_dhcpv6", "interface", section_name(s), &dmmap_s);
...
{
	dmuci_get_value_by_section_string(((struct dmmap_dup *)data)->dmmap_section, "bbf_dhcpv6client_alias", value);
	if ((*value)[0] == '\0')
...
static int set_DHCPv6Client_Alias(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	switch (action)	{
		case VALUECHECK:
			...
			break;
		case VALUESET:
			dmuci_set_value_by_section(((struct dmmap_dup *)data)->dmmap_section, "bbf_dhcpv6client_alias", value);
			break;
	}
	return 0;
}
...
static int get_DHCPv6Client_RequestAddresses(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	dmuci_get_value_by_section_string(((struct dmmap_dup *)data)->config_section, "reqaddress", &reqaddress);
	*value = DM_LSTRCMP(reqaddress, "none") == 0 ? "0" : "1";
	return 0;
}
...
static int set_DHCPv6Client_RequestAddresses(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	...
	switch (action)	{
		...
		case VALUESET:
			string_to_bool(value, &b);
			dmuci_set_value_by_section(((struct dmmap_dup *)data)->config_section, "reqaddress", b ? "force" : "none");
			break;
	}
	return 0;
}
...
static int get_DHCPv6Client_RequestPrefixes(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	dmuci_get_value_by_section_string(((struct dmmap_dup *)data)->config_section, "reqprefix", &reqprefix);
...
static int set_DHCPv6Client_RequestPrefixes(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	...
	switch (action)	{
		...
		case VALUESET:
			string_to_bool(value, &b);
			dmuci_set_value_by_section(((struct dmmap_dup *)data)->config_section, "reqprefix", b ? "auto" : "no");
			return 0;
	}
	return 0;
}
...
{
	dmuci_get_value_by_section_string(((struct dmmap_dup *)data)->config_section, "reqopts", value);
	return 0;
}
...
static int set_DHCPv6Client_RequestedOptions(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	switch (action)	{
		case VALUECHECK:
			...
			break;
		case VALUESET:
			dmuci_set_value_by_section(((struct dmmap_dup *)data)->config_section, "reqopts", value);
			break;
	}
	return 0;
}
